'''This file contains the implementation code for custom layers for the CBAM-UNet model.
'''

import tensorflow as tf
import tensorflow.keras as keras
from tensorflow.keras.layers import Conv2D, ReLU, MaxPooling2D, Conv2DTranspose, Layer, Lambda


class Encoder_ConvBlock(Layer):
    
    '''Convolutional block for the encoder (downsampling portion) of the model. 
    '''
    
    def __init__(self, num_filters, kernel_size, strides, padding, **kwargs):
        
        super(Encoder_ConvBlock, self).__init__(**kwargs)
        
        self.conv_layer1 = Conv2D(filters = num_filters, kernel_size = kernel_size, strides = strides, padding = 'same')
        self.conv_layer2 = Conv2D(filters = num_filters, kernel_size = kernel_size, strides = strides, padding = 'same')
    
    
    def call(self, x):
            
        x = self.conv_layer1(x)
        x = ReLU()(x)
        x = self.conv_layer2(x)
        x = ReLU()(x)
            
        return x

    
class Decoder_ConvBlock(Layer):
    
    '''Convolutional block for the encoder (downsampling portion) of the model.
    '''
    
    def __init__(self, num_filters, kernel_size, strides, padding, **kwargs):
        
        super(Decoder_ConvBlock, self).__init__(**kwargs)
        
        self.conv_layer1 = Conv2D(filters = num_filters, kernel_size = kernel_size, strides = strides, padding = 'same')
        self.conv_layer2 = Conv2D(filters = num_filters, kernel_size = kernel_size, strides = strides, padding = 'same')
        
    def call(self, x):
        
        x = self.conv_layer1(x)
        x = ReLU()(x)
        x = self.conv_layer2(x)
        x = ReLU()(x)

        return x

    
class Output_ConvBlock(Layer):
    
    '''Convolutional block after the decoder portion of the model (final convolution layers).
    '''
    
    def __init__(self, num_filters, kernel_size, strides, padding, **kwargs):
        
        super(Output_ConvBlock, self).__init__(**kwargs)
        
        self.conv_layer1 = Conv2D(filters = num_filters, kernel_size = kernel_size, strides = strides, padding = 'same')
        self.conv_layer2 = Conv2D(filters = num_filters, kernel_size = kernel_size, strides = strides, padding = 'same')
        self.conv1d_layer = Conv2D(filters = 1, kernel_size = 1, activation = 'sigmoid')
    
    def call(self, x):
        
        x = self.conv_layer1(x)
        x = self.conv_layer2(x)
        x = self.conv1d_layer(x)
        
        # Only threshold when generating segmentation map, don't use it in model as it will
        # make loss non differentiable
        # threshold = 0.5
        # x = Lambda(lambda val: tf.where(val >= threshold, 1.0, 0.0))(x)
        
        return x