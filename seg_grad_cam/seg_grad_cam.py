...
# A set of pixel indices of interest in the output mask.
...
class SegGradCAM:
    ...
    def featureMapsGradients(self):
        ...
        # Explanation: Make image shape compatible with batch input
        preprocessed_input = np.expand_dims(self.image, 0)
        preprocessed_gt = np.expand_dims(self.gt, 0)        
        
        with tf.GradientTape() as gtape:
            
            # Get output of the layer we want to calculate the gradient wrt to
            conv_layer = self.input_model.get_layer(self.prop_to_layer)
            self.watch_layer(conv_layer, gtape)
            
            # Get output of the final layer 
            
            # original: (take the outout of the final layer and apply a mask on areas to focus on
        ...
        # Resize and store the output and gradients
        self.A = conv_layer.result[0, ...]  # Assuming the batch dimension is 0 so get all feature maps
        self.grads_val = grads[0, ...] if grads is not None else None # get gradients for all feature map
    ...
    def activationMap(self):
        ...
        # Explanation: This line computes the weighted sum of the feature maps (self.A) based on the calculated weights (self.alpha_c). Each element of self.A corresponds to a feature map, and each element of self.alpha_c corresponds to the weight of the corresponding feature map for the given class. The dot product performs the summation of these weighted feature maps.
        # weighted sum of feature maps: sum of alpha^k_c * A^k
        cam = np.dot(self.A, self.alpha_c)  # *abs(grads_val) or max(grads_val,0)
        
        # This line resizes the computed activation map (cam) to the dimensions of the input image (self.image). It uses the OpenCV function cv2.resize with bilinear interpolation (cv2.INTER_LINEAR).
